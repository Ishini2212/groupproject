...
if (isset($_POST["submit"])) {
    ...
    $adminresult = mysqli_query($conn, "SELECT * FROM teacher WHERE name = '$user'"); //connection to admins
    ...
    if (mysqli_num_rows($adminresult) > 0) { //admin validation
        if ($password == $adminrow["password"]) {
            ...
            $_SESSION["teacher_id"] = $adminrow["teacher_id"];
...
<html lang="en">
    <head>
        ...
        <title>SkySchool - Teacher login</title>
